...
export default function AdminPage() {
    ...
    // States for Capacity Config
    const [hourlyCapacity, setHourlyCapacity] = useState(50);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const fetchedCapacity = await getHourlyCapacityAction();
            const fetchedOccupancy = await getOccupancyReportAction(selectedReportDate);
            setOrders(fetchedOrders);
            setProducts(fetchedProducts);
            setArticles(fetchedArticles);
            setHourlyCapacity(fetchedCapacity);
            setOccupancyReport(fetchedOccupancy);
        } catch (e) {
    ...
    // Actualizar reporte de aforo si cambia la fecha
    useEffect(() => {
    ...
    const handleUpdateCapacity = async (e: React.FormEvent) => {
        ...
        const res = await updateHourlyCapacityAction(hourlyCapacity);
        if (res.success) {
            setCapacitySuccessMsg("Aforo por hora actualizado correctamente.");
        } else {
                                    ...
                                    <form onSubmit={handleUpdateCapacity} className="space-y-4">
                                        <div className="space-y-1">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setHourlyCapacity(parseInt(e.target.value, 10))}
                                                min={1}
                                                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl font-switzer text-sm focus:outline-none focus:border-primary text-foreground"
                                                required
                                            />
                                ...
                                <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
                                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                        ...
                                        <div>
                                            <input
                                                ...
                                                className="px-4 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl font-switzer text-sm focus:outline-none focus:border-primary text-foreground"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3 pt-4">
                                        {["10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00"].map((slot) => {
                                            ...
                                            const pct = Math.min(100, (booked / hourlyCapacity) * 100);
                                                ...
                                                <div key={slot} className="space-y-1">
                                                    <div className="flex justify-between text-sm font-switzer">
                                                        ...
                                                        <span className="text-slate-500">
                                                            {booked} / {hourlyCapacity} personas ({pct.toFixed(0)}%)
                                                        </span>
                                                    </div>
